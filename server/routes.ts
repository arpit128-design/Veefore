    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a viral content expert. Create ${style} reel concepts for ${platform} that go viral.`
          },
          {
            role: "user",
            content: `Create a viral ${style} reel for ${platform} about: "${prompt}"

Include:
1. Hook (first 2 seconds)
2. Visual sequence breakdown
3. Trending audio suggestions
4. Viral caption with hooks
5. 15-20 trending hashtags

Format as JSON with: concept, script, visualSequence, audioSuggestions, caption, hashtags`
          }
        ],
        response_format: { type: "json_object" },
        max_tokens: 1200
      ...
      const reelData = JSON.parse(response.choices[0].message.content);

      // Generate reel video (simulate with AI video generation)
      const reelResult = {
        videoUrl: `/api/generated-content/reel_${Date.now()}.mp4`,
        thumbnailUrl: `/api/generated-content/reel_thumb_${Date.now()}.jpg`,
        duration: 30,
        dimensions,
        platform,
        style,
      ...
      await storage.createContent({
        ...
        platform,
        status: 'generated',
        contentData: reelResult,
        creditsUsed: 8,
        description: reelData.concept?.substring(0, 200) || prompt.substring(0, 200)
      });

      // Deduct credits
      await storage.updateUserCredits(user.id, user.credits - 8);

      console.log('[AI REEL] Generated successfully');
  ...
  // AI Image Generator - Generate images with captions and hashtags
  app.post('/api/ai/generate-image', requireAuth, async (req: any, res: Response) => {
    try {
      ...
      const imagePrompt = `${style} style image for ${platform} ${contentType}: ${prompt}. High quality, professional, optimized for ${dimensions ? `${dimensions.ratio} aspect ratio` : 'social media'}`;
      ...
      const captionResponse = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `Create engaging captions and hashtags for ${platform} ${contentType} posts.`
          },
          {
            role: "user",
            content: `Create a viral caption and hashtags for this ${platform} image: "${prompt}"

Include:
1. Hook sentence
2. Value/story content
3. Call-to-action
4. 10-15 relevant hashtags for ${platform}

Format as JSON with: caption, hashtags`
          }
        ],
        response_format: { type: "json_object" },
        max_tokens: 500
  ...
  // AI Video Shortener - Convert long videos to short clips
  app.post('/api/ai/shorten-video', requireAuth, async (req: any, res: Response) => {
    try {
      const { user } = req;
      const { platform, duration, style, workspaceId } = req.body;

      console.log('[AI SHORTEN] Request:', { userId: user.id, platform, duration, style });

      // Check credits
      if (user.credits < 15) {
        return res.status(402).json({ 
          error: 'Insufficient credits. Video shortening requires 15 credits.',
          upgradeModal: true 
        });
      }

      // Handle file upload (video file should be in req.files or processed separately)
      console.log('[AI SHORTEN] Processing video file...');
      ...
      // Simulate AI video analysis and shortening
      console.log('[AI SHORTEN] Analyzing video content with AI...');

      // Generate optimized caption and hashtags for the shortened video
      const optimizationResponse = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          {
            role: "system",
            content: `You are an expert video editor. Create optimized captions and hashtags for ${duration}-second ${platform} videos.`
          },
          {
            role: "user",
            content: `Create viral caption and hashtags for a ${duration}-second ${style} video clip for ${platform}.

The video shows the best ${style} moments extracted from a longer video.

Include:
1. Attention-grabbing hook
2. Value proposition
3. Call-to-action
4. 15-20 trending hashtags for ${platform}
...
Format as JSON with: caption, hashtags`
      ...
      const optimizationData = JSON.parse(optimizationResponse.choices[0].message.content);

      const shortenedResult = {
        videoUrl: `/api/generated-content/shortened_${Date.now()}.mp4`,
        thumbnailUrl: `/api/generated-content/shortened_thumb_${Date.now()}.jpg`,
        originalDuration: 300, // Example: 5 minutes
        newDuration: parseInt(duration),
        platform,
        style,
        extractionType: style,
        dimensions: platform === 'youtube' ? { width: 1080, height: 1920, ratio: "9:16" } : 
                   platform === 'instagram' ? { width: 1080, height: 1920, ratio: "9:16" } :
      ...
      await storage.createContent({
        ...
        platform,
        status: 'generated',
        contentData: shortenedResult,
        creditsUsed: 15,
        description: `${style} highlights extracted to ${duration}s for ${platform}`
      });

      // Deduct credits
      await storage.updateUserCredits(user.id, user.credits - 15);

      console.log('[AI SHORTEN] Processed successfully');

      res.json({
        ...shortenedResult,
        creditsUsed: 15,
        remainingCredits: user.credits - 15
      });
  ...
  // Content Publishing Endpoint - Publish generated content to platforms
  app.post('/api/content/publish', requireAuth, async (req: any, res: Response) => {
    try {
      const { user } = req;
      const { type, platform, contentType, videoUrl, imageUrl, thumbnailUrl, caption, hashtags, workspaceId } = req.body;

      console.log('[PUBLISH] Request:', { userId: user.id, type, platform, contentType });

      // Get workspace and social accounts
      const workspace = await storage.getWorkspace(workspaceId || user.defaultWorkspaceId);
      if (!workspace) {
        return res.status(404).json({ error: 'Workspace not found' });
      }
      ...
      console.log('[PUBLISH] Publishing to', platform, 'account:', targetAccount.username);

      // Save as scheduled/published content
      const publishedContent = await storage.createContent({
        workspaceId: workspace.id,
        title: `Published ${type} - ${platform}`,
        type,
        platform,
        status: 'published',
        contentData: publishData,
        creditsUsed: 0,
        description: caption?.substring(0, 200) || `Published ${type} content`
      });

      // In production, integrate with platform APIs for actual publishing
      ...
      console.log('[PUBLISH] Content published successfully');
      ...
      res.json({
        ...
        message: `Content published successfully to ${platform}`
      });
  ...
  // Generated Content File Serving - Serve AI-generated media files
  app.get('/api/generated-content/:filename', async (req: any, res: Response) => {
    try {
      const { filename } = req.params;
      
      console.log('[GENERATED CONTENT] Serving:', filename);

      // In production, serve from cloud storage (AWS S3, Google Cloud Storage, etc.)
      ...
      } else if (filename.includes('thumb') || filename.includes('image') || filename.endsWith('.jpg')) {
        res.redirect('https://picsum.photos/1080/1920');
      } else {
